...
const getMovieDatails = async (id) =>{
  const options = {
    method: 'GET',
    headers: {
      accept: 'application/json',
  ...
  const response = await fetch(`https://api.themoviedb.org/3/movie/${id}?append_to_response=videos&language=pt-BR`, options)
    const data = await response.json();
    return data;
}

export {getMovies,getGenres,getMovieDatails};